...
//       
//       Creation Date:  Wed Sep 14 09:29:28 2022
//       Version: 	r1p1
//
...
`ifdef POWER_PINS
module RAMSP128X16 (VDDCE, VDDPE, VSSE, Q, CLK, CEN, WEN, A, D, EMA, EMAW, GWEN, RET1N);
`else
module RAMSP128X16 (Q, CLK, CEN, WEN, A, D, EMA, EMAW, GWEN, RET1N);
`endif
...
`endif
  ...
  reg NOT_WEN0, NOT_A6, NOT_A5, NOT_A4, NOT_A3, NOT_A2, NOT_A1, NOT_A0, NOT_D15, NOT_D14;
 ...
 wire  CLK_;
  ...
  buf B62(GWEN_, GWEN);
  buf B63(RET1N_, RET1N);

   `ifdef ARM_FAULT_MODELING
     RAMSP128X16_error_injection u1(.CLK(CLK_), .Q_out(Q_), .A(A_int), .CEN(CEN_int), .GWEN(GWEN_int), .WEN(WEN_int), .Q_in(Q_int));
  `else
...
`endif
`timescale 1ns/1ps
module RAMSP128X16_error_injection (Q_out, Q_in, CLK, A, CEN, WEN, GWEN);
   output [15:0] Q_out;
   input [15:0] Q_in;
   input CLK;
   input [6:0] A;
