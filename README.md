...
本仓库沉淀了嵌入式竞赛项目中实际应用的数字 IC 物理设计脚本与资料，采用 Tcl 和 Shell 组织，覆盖 RTL 综合、形式等价验证、物理实现、寄生参数提取、静态时序分析、物理验证、功耗/电迁移分析及部分签核检查。
...
> [!IMPORTANT]
> 本流程已在嵌入式竞赛项目中实际应用。为保留不同阶段的实践记录和参考配置，仓库中仍存在多个设计版本、外部 Linux 路径及机器相关设置；在新的环境中复现、迁移或跨阶段串联时，需核对并统一设计名、输入输出文件名、工艺库、角点、场景及上下游路径。
...
本仓库保留了竞赛项目实施过程中不同阶段、不同设计版本的脚本和配置，因此当前检出的默认设置并非同一设计的一套连续配置：
...
因此，复现某次竞赛项目运行结果或在新环境中继续流程时，应先选定对应设计版本，再核对以下内容：
