...
create_clock -name ref_clk -period ${CLOCK_PERIOD_REFCLK} [get_ports ap_clk]
...
set data_inputs [remove_from_collection $input_ports [get_ports ap_clk]]
...
set_dont_touch          [get_ports ap_clk]
set_ideal_network       [get_ports ap_clk]
set_dont_touch          [get_ports ap_rst_n]

#复位信号到内部逻辑之间的路径被标记为“假路径”，不参与时序分析
set_false_path -from [get_ports ap_rst_n] -to [all_registers]
