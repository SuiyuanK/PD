...
//       
//       Creation Date:  Thu Sep 15 08:41:14 2022
//       Version: 	r1p1
//
...
`ifdef POWER_PINS
module RAMTP128X16 (VDDCE, VDDPE, VSSE, QA, CLKA, CENA, AA, CLKB, CENB, WENB, AB, DB,
    EMAA, EMAB, RET1N, COLLDISN);
`else
module RAMTP128X16 (QA, CLKA, CENA, AA, CLKB, CENB, WENB, AB, DB, EMAA, EMAB, RET1N,
    COLLDISN);
`endif
...
`endif
  ...
  begin
    if (RET1N_int === 1'bx || RET1N_int === 1'bz) begin
      ...
      QA_int = {data_out[30], data_out[28], data_out[26], data_out[24], data_out[22],
  ...
  begin
    if (RET1N_int === 1'bx || RET1N_int === 1'bz) begin
      ...
      new_data =  ( {1'b0, DB_int[15], 1'b0, DB_int[14], 1'b0, DB_int[13], 1'b0, DB_int[12],
...
`endif
 ...
 wire  CLKB_;
  ...
  buf B25(CLKB_, CLKB);
  buf B26(CENB_, CENB);
  buf B27(WENB_[0], WENB[0]);
  buf B28(WENB_[1], WENB[1]);
  buf B29(WENB_[2], WENB[2]);
  buf B30(WENB_[3], WENB[3]);
  buf B31(WENB_[4], WENB[4]);
  buf B32(WENB_[5], WENB[5]);
  buf B33(WENB_[6], WENB[6]);
  buf B34(WENB_[7], WENB[7]);
  buf B35(WENB_[8], WENB[8]);
  buf B36(WENB_[9], WENB[9]);
  buf B37(WENB_[10], WENB[10]);
  buf B38(WENB_[11], WENB[11]);
  buf B39(WENB_[12], WENB[12]);
  buf B40(WENB_[13], WENB[13]);
  buf B41(WENB_[14], WENB[14]);
  buf B42(WENB_[15], WENB[15]);
  buf B43(AB_[0], AB[0]);
  buf B44(AB_[1], AB[1]);
  buf B45(AB_[2], AB[2]);
  buf B46(AB_[3], AB[3]);
  ...
  buf B58(DB_[8], DB[8]);
  buf B59(DB_[9], DB[9]);
  buf B60(DB_[10], DB[10]);
  buf B61(DB_[11], DB[11]);
  buf B62(DB_[12], DB[12]);
  buf B63(DB_[13], DB[13]);
  buf B64(DB_[14], DB[14]);
  buf B65(DB_[15], DB[15]);
  buf B66(EMAA_[0], EMAA[0]);
  buf B67(EMAA_[1], EMAA[1]);
  buf B68(EMAA_[2], EMAA[2]);
  buf B69(EMAB_[0], EMAB[0]);
  buf B70(EMAB_[1], EMAB[1]);
  buf B71(EMAB_[2], EMAB[2]);
  buf B72(RET1N_, RET1N);
  buf B73(COLLDISN_, COLLDISN);

   `ifdef ARM_FAULT_MODELING
     RAMTP128X16_error_injection u1(.CLK(CLKA_), .Q_out(QA_), .A(AA_int), .CEN(CENA_int), .Q_in(QA_int));
  `else
...
`endif
`timescale 1ns/1ps
module RAMTP128X16_error_injection (Q_out, Q_in, CLK, A, CEN);
   output [15:0] Q_out;
   input [15:0] Q_in;
   input CLK;
   input [6:0] A;
