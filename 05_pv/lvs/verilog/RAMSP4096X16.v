...
//       
//       Creation Date:  Wed Sep 14 10:33:00 2022
//       Version: 	r1p1
//
...
`ifdef POWER_PINS
module RAMSP4096X16 (VDDCE, VDDPE, VSSE, Q, CLK, CEN, WEN, A, D, EMA, EMAW, GWEN, RET1N);
`else
module RAMSP4096X16 (Q, CLK, CEN, WEN, A, D, EMA, EMAW, GWEN, RET1N);
`endif
...
`endif
  ...
  reg NOT_A2, NOT_A1, NOT_A0, NOT_D15, NOT_D14, NOT_D13, NOT_D12, NOT_D11, NOT_D10;
  reg NOT_D9, NOT_D8, NOT_D7, NOT_D6, NOT_D5, NOT_D4, NOT_D3, NOT_D2, NOT_D1, NOT_D0;
  reg NOT_EMA2, NOT_EMA1, NOT_EMA0, NOT_EMAW1, NOT_EMAW0, NOT_GWEN, NOT_RET1N;
 ...
 wire  CLK_;
  ...
  buf B60(D_[14], D[14]);
  buf B61(D_[15], D[15]);
  buf B62(EMA_[0], EMA[0]);
  buf B63(EMA_[1], EMA[1]);
  buf B64(EMA_[2], EMA[2]);
  buf B65(EMAW_[0], EMAW[0]);
  buf B66(EMAW_[1], EMAW[1]);
  buf B67(GWEN_, GWEN);
  buf B68(RET1N_, RET1N);

   `ifdef ARM_FAULT_MODELING
     RAMSP4096X16_error_injection u1(.CLK(CLK_), .Q_out(Q_), .A(A_int), .CEN(CEN_int), .GWEN(GWEN_int), .WEN(WEN_int), .Q_in(Q_int));
  `else
...
`endif
`timescale 1ns/1ps
module RAMSP4096X16_error_injection (Q_out, Q_in, CLK, A, CEN, WEN, GWEN);
   output [15:0] Q_out;
   input [15:0] Q_in;
   input CLK;
   input [11:0] A;
...
begin
   ...
   `ifdef ARM_NONREPAIRABLE_FAULT
      `pre_pend_path.SMARCHCHKBVCD_LVISION_MBISTPG_ASSEMBLY_UNDER_TEST_INST.MEM0_MEM_INST.u1.add_fault(12'd3718,4'd6,2'd1,2'd0);
   `endif
end
   task add_fault;
