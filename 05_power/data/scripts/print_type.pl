...
################################################################################################################
# Name       : print_type.pl
# Description : Prints % switching for different types / nets / locations  / variable time span / resolution
...
while (<rpt6>)
{
 ...
 $totc=0;$combc=0;$latchc=0;$memc=0;$ffc=0;$clk_bufc=0;$cell_w_clk_c=0;
foreach $key(sort keys %total_counter){
if ($counter1{$key} != 0 ) { $combc++;}
elsif ($counter2{$key} != 0 ) { $latchc++;}
elsif ($counter3{$key} != 0 ) { $memc++;}
elsif ($counter4{$key} != 0 ) { $ffc++;}
elsif ($counter5{$key} != 0 ) { $clk_bufc++;}
elsif ($counter6{$key} != 0 ) { $cell_w_clkc++;}
else { $remainin++;}
}
$totc=$combc+$latchc+$memc+$ffc+$clk_bufc+$cell_w_clkc;


printf"\n \n \n";

print " \n ---------------------------------------------------------------------------------------------\n";
printf "%20s\t%20s\t%20s\t%20s","Type","Switching","Total","Switch %";
print " \n ---------------------------------------------------------------------------------------------\n";
if ($COMB_counter != 0)
{
$per_comb = ( $combc/$COMB_counter)*100;
$per_comb = sprintf "%.2f",$per_comb;
}
else { $COMB_counter = 0;$per_comb=0;$combc=0;}
printf "%20s\t%20s\t%20s\t%20s","Combinational","$combc","$COMB_counter","$per_comb %\n";


if ( $FF_counter != 0)
{
$per_FF = ( $ffc/$FF_counter)*100;
$per_FF = sprintf "%.2f",$per_FF;
}
else { $FF_counter = 0;$per_FF=0;$ffc=0;}
printf "%20s\t%20s\t%20s\t%20s","Flipflop","$ffc","$FF_counter","$per_FF %\n";


if ( $LATCH_counter != 0)
{
$per_latch  = ( $latchc/$LATCH_counter)*100;
$per_latch = sprintf "%.2f",$per_latch;
}
else { $LATCH_counter = 0;$per_latch=0;$latchc=0;}
printf "%20s\t%20s\t%20s\t%20s","Latch","$latchc","$LATCH_counter","$per_latch %\n";

if ($MEM_counter != 0)
{
$per_mem = ( $memc/$MEM_counter)*100;
$per_mem = sprintf "%.2f",$per_mem;
}
else { $MEM_counter = 0;$per_mem=0;$memc=0;}
printf "%20s\t%20s\t%20s\t%20s","Memory","$memc","$MEM_counter","  $per_mem %\n";


if ($CLK_BUF_counter != 0)
{
$per_clk_buf = ( $clk_bufc/$CLK_BUF_counter)*100;
$per_clk_buf = sprintf "%.2f",$per_clk_buf;
}
else { $CLK_BUF_counter = 0;$per_clk_buf=0;$clk_bufc=0;}
printf "%20s\t%20s\t%20s\t%20s","Clock_buf","$clk_bufc","$CLK_BUF_counter","$per_clk_buf %\n";

if ($CELL_W_CLK_counter != 0)
{
$per_clk = ($cell_w_clkc/$CELL_W_CLK_counter)*100;
$per_clk = sprintf "%.2f",$per_clk;
}
else { $CELL_W_CLK_counter = 0;$per_clk=0;$cell_w_clkc=0;}
printf "%20s\t%20s\t%20s\t%20s","Cell with clock","$cell_w_clkc","$CELL_W_CLK_counter","$per_clk %\n";


if ($TOT_counter != 0)
{
$per_tot = ( $totc/$TOT_counter)*100;

$per_tot = sprintf "%.2f",$per_tot;
}
else { $TOT_counter = 0;$per_tot=0;}
printf "%20s\t%20s\t%20s\t%20s","Total","$totc","$TOT_counter"," $per_tot %\n";

print " \n ---------------------------------------------------------------------------------------------\n";
#print " \n ff= $FF_counter \n latcfh = $LATCH_counter \n  $MEM_counter \n $CELL_W_CLK_counter \n $CLK_BUF_counter \n $COMB_counter";
print " \n $TOT_counter out of $countt instances selected.Rest of the cells may be Non-functional cells like Decap / Filler cells etc. \n\n";
if($remainin ne '0' )
{
print " \n $remainin instances couldn't be mapped into 6 of the above catagories & have missing .libs\n";}
close ptr1;
close ptr2;
close ptr3;
close ptr4;
