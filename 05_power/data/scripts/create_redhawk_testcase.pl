...
foreach $gsr_key (@GSR_keywords) {
  ...
  if (($gsr_key =~ m/_FILE/) || ($gsr_key =~ m/GDS_CELLS/) || ($gsr_key =~ m/EM_TECH_/) || ($gsr_key =~ m/PACKAGE_SPICE_SUBCKT/)  || ($gsr_key =~ m/VP_CONTROL/) || ($gsr_key =~ m/STATE_PROPAGATION/) || ($gsr_key =~ m/RDL_CELL/) || ($gsr_key =~ m/ERV_CELLS/) || ($gsr_key =~ m/CMM_CELLS/)) {
	...
	if (-l "send2apache/design_data/$target_dir/$basename") {
	  $basename = resolveSameBaseName("send2apache/design_data/$target_dir/$basename");
	}
	edaExecute("\\ln -s $file send2apache/design_data/$target_dir/$basename");
	$total_byte_count += (stat($file))[7];
	print OUT_GSR "@line ../design_data/$target_dir/$basename \n";
	next;		
      }
	...
	if($word =~ /\.defs$/ || $word =~ /\.lefs$/ || $word =~ /\.libs$/ ) {
		open INFS , "$word" or open INFS , "$cmd_dirname/$word" or print "Cannot open $word and $cmd_dirname/$word for reading :$!\n";
		while(<INFS>) {
			chomp;split;
			#if(/_FILE/) { next;}
			if (-r "$cmd_dirname/$_[0]") {
	  			$file = edaConvertToAbsolutePath("$cmd_dirname/$_[0]");
			} elsif (-r "$_[0]") {
	  			$file = edaConvertToAbsolutePath("$_[0]");
			} else {
				next;
			}
	 		$basename = basename($file);
	...
	if (-r "$cmd_dirname/$word") {
	  ...
	  if($debug_mode) {
	    print "DEBUG: $cmd_dirname/$word is readable\n";
	  }
	...
	} elsif (-r "$word") { # can be absolute path to the file
	  # Just in case it was a manual GSR - convert to absolute path if $word is already not absolute path,ie, $word does not start with /
	  ...
	  if (-l "send2apache/design_data/$target_dir/$basename") {
	   $basename = resolveSameBaseName("send2apache/design_data/$target_dir/$basename");
	    ...
	    if ($gsr_key =~ m/GDS_CELLS/) {
	      ...
	      # $prev_word contains the name of the memory 
	      # This covers both 5.2 and 5.3 gdsmem
	      foreach $gds_file (glob("$file/$prev_word*{lef,def,lib,pratio}")) {
		edaExecute("\\ln -s $gds_file send2apache/design_data/$target_dir");
		$total_byte_count += (stat("$gds_file"))[7];		
	      }
	      print OUT_GSR "../design_data/$target_dir ";
...
my $count = 1;
(my $basenameold = $_[0]) =~ s!^.*/!!;
my $basename = "${basenameold}_${count}";
	while (-l "send2apache/design_data/$target_dir/$basename") {
	$count++;
