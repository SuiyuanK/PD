proc atclQueryEM_help {} {
puts "atclQueryEM -setup <conf file name> ?-window <llx lly urx ury>? ?-nets <net names>? ?-layers <layer names>? ?-o <output file name>? ?-h? ?-m?"
}

proc atclQueryEM_manpage {} {
puts "
SYNOPSIS
        Apache-AE TCL utility to get EM info on wires/vias
                                                                                                                                                            
USAGE
	atclQueryEM -setup <conf file name> ?-window <llx lly urx ury>? ?-nets <net names>? ?-layers <layer names>? ?-o <output file name>? ?-h? ?-m?
   Options:
...
#Step 1# Setup the script
   Example Command 
   	atclQueryEM -setup layers.conf

#Step 2# Run script using other options
   Example Commands 
   	atclQueryEM -o emoutput.txt
	atclQueryEM -window 0 0 100 100
	atclQueryEM -window 0 0 100 100 -nets vdd1 vdd2 vss
	atclQueryEM -window 0 0 100 100 -nets vdd1 vdd2 vss -layers metal1 metal2 metal3
	atclQueryEM -window 0 0 100 100 -nets vdd1 vdd2 vss -layers metal1 metal2 metal3 -o emoutput.txt
...
For Wire-Segments:
<layer> <net> <llx lly urx ury> <dir> <current> <em_limit> <em> <resistance> <segmentid>

For Vias:
<id> <layer> <vianame> <net> <cordx cordy> <cuts_#> <cut_width> <cut_height> <resistance> <dir> <current> <em_limit> <em> <top_metal_id> <bot_metal_id> <top_seg_id1> <top_seg_id2> <bot_seg_id1> <bot_seg_id2> <top_metal llx lly urx ury> <via_cut llx lly urx ury> <bot_metal llx lly urx ury>

"
}


proc atclQueryEM { args } {
set argv [split $args]
...
for {set j 0} {$j < $argc} {incr j 1} {
	if {[regexp {\-setup} [lindex $argv $j]]} {
		...
		set outflag 1
                ...
                atclQueryEM_help;return
        } elseif  {[regexp {\-m} [lindex $argv $j]]} {
                atclQueryEM_manpage;return
        }
}
...
if {$setupflag} {
	atclQueryEM_Setup $conffile
} else {
	atclQueryEM_Report $args
}

}; ### End of proc atclQueryEM

proc atclQueryEM_Setup {conffile} {
config cmdlog off
...
puts "Setting up for atclQueryEM"
...
set file [glob -nocomplain -directory . .apache/.atclQueryEMDB.db]
if {[file exists $file]} {
...
sqlite3 db .apache/.atclQueryEMDB.db
...
if {![file exists .apache/.atclQueryEMDB.db]} {
...
}; ### End of proc atclQueryEM_Report
