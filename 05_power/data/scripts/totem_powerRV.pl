...
$file_apl    = "".$apl_mmx_path."/adsRpt/aplmmx.log";
if(-e $file_apl)
{
open(FP1, $file_apl);
while(<FP1>)
...
if($var[35] =~/DYNA.*/)
{
$file_emavg  = './adsRpt/Dynamic/'.$var[19].'.em.worst';
$file_emrms = './adsRpt/Dynamic/'.$var[19].'.em.worst.rms';
#$file_emshort ='./adsRpt/Dynamic/'.$var[19].'.em.worst.short';
$file_emlong ='./adsRpt/Dynamic/'.$var[19].'.em.worst.peak';

}
else
{
$file_emavg  = './adsRpt/Static/'.$var[19].'.em.worst';
$file_emrms = './adsRpt/Static/'.$var[19].'.em.worst.rms';
#$file_emshort ='./adsRpt/Static/'.$var[19].'.em.worst.short';
$file_emlong ='./adsRpt/Static/'.$var[19].'.em.worst.peak';
}

$file_sigemnet = './adsRpt/apache.sigem.netinfo';
$file_totcl = './totcl.txt';
#$file_mactype = './gdsmmx/'.$var[19].'.gds.config';
$file_mactype = './'.$var[19].'.gds.config'; 
...
close(FP1);
print "Finished reading APLMMX configuration file..\n";
}

#else
#{
#print "Cannot find/open".$file_apl."\n";
#}
...
#else 
#{
#print "Cannot open/find".$file_apl."\n" ;
#}
###############################################
...
print "Finished reading the EM report files...\n";
...
if ($_ !~m/^#/)
{
@array_ipwr = split(/\s+/,$_);
push(@array_dcurrent, $array_ipwr[1]);
}
}
close(FP9);
}
#else
#{
#print "Cannot open/find".$file_ipwr."\n";
#}
@array_dcurrent = sort { $a <=> $b } @array_dcurrent;
$max1 = $array_dcurrent[$#array_dcurrent];
$var[42] = $max1;
if(-e $file_ivdd)
{
open(FP10,$file_ivdd);
...
if ($_ !~m/^#/)
{
@array_ivdd = split(/\s+/,$_);
push(@array_bcurrent,$array_ivdd[1]);
}
}

close(FP10) or die "Cannot close the file";
}
#else
#{
#print "Cannot open/find".$file_ivdd."\n" ;
#}
@array_bcurrent = sort { $a <=> $b } @array_bcurrent;
$max2 = $array_bcurrent[$#array_bcurrent];
$var[43] = $max2;

print "Finished reading the current profiles..\n";
if(-e $threshold_file)
{
...
close(FP12);
print "Finished reading the S-factor report..\n";
}
...
print FP2 "NO OF VOLTAGE DOMAINS   \t" .$length."\n";
...
print FP2 "Total APLMMX time            ".$var[17]."\n";
...
print FP2 "\nIR RESULTS SUMMARY\n" ;
print FP2 "----------------------------------------------------------------------------------------------\n";
print FP2 "Total Chip Power Calculated   ".$var[25]." Watt\n";
$e =0 ;
$varq=0 ;
$b = 0;
open(F4, $file_totlog);
...
while($k = <F4>)
{
if ($varq == 1) {@as = split(" ",$k) ; $vol = $as[2] ;
print FP2 "WIRE($b) DROP                ".$as[2]."\n";
$varq = 0; 
}
##
if ($k=~m/^NET<(\S+)>/) {
...
$e=0 ;
while ( $e < $length ) {
###if ($varq == 1 ) { print FP2 "WIRE($b) DROP      ".$k."\n"; $varq = 0; next ;}
chomp ;
#print FP2 "Domain name $e: $domains[$e]\n";
#
@field=split (" ",$domains[$e] );
##$a=$field[0] ;
###print FP2 "$a\n" ;
if ($field[0] eq $b ) {
#print FP2 "While loop print :$k\n" ;
$varq=1 ;
#next ;
} 
$e++ ;
...
close (F4); 
print FP2 "WIRE(VSS) DROP                ".$var[27]." mV\n";
print FP2 "Xtor(VDD) DROP                ".$var[28]." mV\n";
print FP2 "Xtor(VSS) DROP                ".$var[29]." mV\n";
print FP2 "Xtor(Minwc VDD)DROP           ".$var[45]." mV\n";
print FP2 "Xtor(Minwc VSS)DROP           ".$var[46]." mV\n";
print FP2 "PKG (VDD) DROP                ".$var[39]." mV\n";
print FP2 "PKG (VSS) DROP                ".$var[40]." mV\n";
print FP2 "MAX SWITCH DROP               ".$var[41]." mV\n";
print FP2 "PEAK DEMAND CURRENT           ".$var[42]." A\n";
print FP2 "PEAK BATTERY CURRENT          ".$var[43]." A\n";
print FP2 "STATIC IR THRESHOLD(pass/fail)".$var[44]." \n\n\n";
#print FP2 "Number of regions failed      ".$var[45]."\n";
#print FP2 "Number of instances failed    ".$var[46]."\n\n\n";
...
if(-e $file_emlong)
{
print FP2 "PEAK-EM VIOLATION THRESHOLD    ".$var[55]." %\n";
print FP2 "TOTAL PEAK-EM VIOLATIONS       ".$var[56]." \n";
print FP2 "WORST METAL PEAK-EM VIOLATIONS ".$var[57]."\n";
print FP2 "WORST VIA PEAK-EM VIOLATIONS   ".$var[58]."\n";
print FP2 "Refer to \n";
...
print FP2 "--------------------------------------------------------\n";
print "Power RV summary report generated successfully!!\n";
